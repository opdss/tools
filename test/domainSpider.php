		...
		foreach ($arr as $c) {
			foreach ($arr as $d) {
				...
				if (substr($original, 0, 3) == '210') {
					file_put_contents('./domainSpider-info-'.$pre.'.log', $dd . ' => ' . $original . PHP_EOL);
					file_put_contents('./domainSpider-success-'.$pre.'.log', $dd . PHP_EOL, FILE_APPEND);
				} else {
					file_put_contents('./domainSpider-info-'.$pre.'.log', $dd . ' => ' . $original . PHP_EOL);
				}
				if ($i++ > 10000) {
