...
class Format extends Base
{
	...
	/**
	 * //@pattern /beautiful.html
	 * @menu 格式化工具|html/css/js美化
	 * @param Request $request
	 * @param Response $response
	 * @param $args
	 */
	public function beautiful(Request $request, Response $response, $args)
	{
		$data['current_menu'] = $this->getCurrentMenu($request);
		$this->addStaticsDir('beautify');
		$this->addStaticsDir('bootstrap-select');
		return $this->view('format/beautiful.twig', $data);
	}

	/**
	 * to_json
	 * @pattern /format/jsonview.html
	 * @menu 格式化工具|json格式化(new)
	 * @param Request $request
	 * @param Response $response
	 * @param $args
	 */
	public function jsonView(Request $request, Response $response, $args)
	{
		$data['current_menu'] = $this->getCurrentMenu($request);
		$this->addStaticsDir('jsonview');
		$this->addStaticsDir('bootstrap-select');
		return $this->view('format/jsonview.twig', $data);
	}
