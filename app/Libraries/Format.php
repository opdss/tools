...
class Format {
     ...
     * @param array $headings  头信息
     * @return string A csv string
     */
    public function to_csv($data = NULL, $delimiter = ',', $enclosure = '"', $headings=array())
    {
        ...
        {
            // Multi-dimensional array
            $headings = (!empty($headings) && (count($headings) == count($data[0])) ? $headings : array_keys($data[0]));
        }
        else
        {
            // Single array
            $headings = (!empty($headings) && (count($headings) == count($data)) ? $headings : array_keys($data));
            $data = [$data];
        }
        // Apply the headings
	...
	function formatJson($json, $unescapeUnicode = false, $unescapeSlashes = false)
	{
		$result = '';
		$pos = 0;
		$strLen = strlen($json);
